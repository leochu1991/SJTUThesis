...
\chapter{统计策略在晶振源补偿和透明时钟中的应用及算法仿真结果}
...
然后，保持上面的阶跃信号不变，采用文中提出的“动态阈值法”来进行优化，可以得到如下的结果如图\ref{fig:添加暂时性时延突变并采用动态阈值法处理后的同步曲线}
\begin{figure}[!hbp]
  \centering
  \begin{minipage}[b]{1\textwidth}
    \captionstyle{\centering}
...
\section{本章小结}
在本章节中，首先针对从时钟晶振源漂移问题进行了分析，然后为了提高主从时钟频率的一致性，文中提出了相关的统计算法，通过在从时钟处累计Sync报文的收发周期，然后统一计算出从时钟的频率漂移量，并对从时钟本地频率进行校正。

然后，为了适应新的ptp设备－透明时钟，文中首先对比了边界时钟与透明时钟的区别，然后将文中的多种统计算法依次进行调整，优化，使得这些统计方法能够更好的适用于透明时钟。

最后，文中通过自己搭建基于stateflow的ptp仿真平台，对于文中提出的统计算法进行了仿真验证，通过最终的仿真结果可以看出统计算法不仅能够很好的提高同步系统对多种时延变化的鲁棒性，更能提高整个系统的同步精度和稳定性。
