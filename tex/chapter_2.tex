...
随后，则会进行时钟同步过程。该过程主要是主从时钟之间发送报文，使得每个从时钟计算出自身与主时钟之间的时钟偏差，并且对自身进行校正从而实现同步。首先主时钟会周期性对外发送Sync报文，该报文会标记离开主时钟的时间戳。当从时钟收到Sync报文后，会记录接受时间戳和发送时间戳。但是，仅仅依靠这两个时间戳不足以计算出偏差，因为从时钟并不知道自身与主时钟之间的链路传输延时。所以，从时钟自身也会周期性向主时钟发送Delay\_Request报文，会通过接受主时钟回传的Delay\_Resopnse报文来共同计算出链路延时delay和主从偏差offset。最终利用offset对从时钟进行校正而实现同步。
...
\begin{figure}[!hbp]
  ...
  \begin{minipage}[b]{0.6\textwidth}
    ...
    \bicaption[fig:longcaptiongood]{时钟同步算法流程图}{时钟同步算法流程图}{Fig}{Method of Time Sync}
  \end{minipage}     
\end{figure}
...
\begin{figure}[!hbp]
  ...
  \begin{minipage}[b]{0.6\textwidth}
    ...
    \bicaption[fig:longcaptiongood]{PTP报文头部格式}{PTP报文头部格式}{Fig}{The Style of PTP message Header}
  \end{minipage}     
\end{figure}
...
\begin{figure}[!hbp]
  ...
  \begin{minipage}[b]{0.6\textwidth}
    ...
    \bicaption[fig:longcaptiongood]{PTP报文主体格式}{PTP报文主体格式}{Fig}{The Style of PTP message Content}
  \end{minipage}     
\end{figure}
